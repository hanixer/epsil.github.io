...
Handlebars.registerHelper('text', function (html) {
  var div = $('<div>')
  div.html(html)
...
var templates = {
  ...
  body:
    ...
    '{{author}}' +
    '{{#if date}}' +
    ' <span style="float: right"><i class="fa fa-calendar-o"></i> <time class="dt-published">{{dateFormat date}}</time></span>' +
    '{{/if}}' +
    '</p>\n' +
    '{{else}}' +
    '{{#if date}}' +
    '<p><i class="fa fa-calendar-o"></i> <time class="dt-published">{{dateFormat date}}</time></p>\n' +
    '{{/if}}' +
    '{{/if}}' +
    '{{else}}' +
    '{{#if date}}' +
